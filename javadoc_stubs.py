...
class JavaClass(JavaObject):
    ...
    def format_as_lines(self, **kwargs):
        ...
        if self.package:
            ...
            ret.append("")
        ret.extend([javadoc_comment(self._desc), self._definition + " {"])
        ret.append("")
...
class JavaMethod(JavaObject):
    ...
    def format_as_lines(self, **kwargs):
        has_body = (
            "abstract" not in self._modifiers and self._parent.type != "interface"
        )
...
class JavaDocParser:
        ...
        with open(file, encoding="utf-8") as f:
            ...
            j_class = JavaClass(
                ...
                name=(
                    ...
                    if header.find("span", {"class": "typeNameLabel"})
                    else "WhyNoClassName"
                ),
                definition=(
                ...
                description=(
                    ...
                    if header.find("div", {"class": "block"})
                    else ""
                ),
            )
            try:
                constructors = soup.find("a", {"name": "constructor.detail"}).parent(
                    "ul", recursive=False
                )
            except AttributeError:
                constructors = []
            ...
            try:
                method_details = soup.find("a", {"name": "method.detail"}).parent(
                    "ul", recursive=False
                )
            except AttributeError:
                method_details = []
            ...
            try:
                fields = soup.find("a", {"name": "field.detail"}).parent.find_all(
                    "ul", recursive=False
                )
            except AttributeError:
                fields = ()
    ...
    def parse_one_field(self, li_elem: bs4.Tag):
        ...
        desc = li_elem.find("div", {"class": "block"})
        if desc is not None:
            j_field.set_description(desc.decode_contents())
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        "INPUT", nargs="+", help="input JavaDoc HTML files or directory"
    )
    arg_parser.add_argument(
        "--output-directory",
        "-o",
    ...
    arg_parser.add_argument(
        "--todo", "-t", action="store_true", help="add TODO comments to methods"
    )
    ...
    for c in out:
        ...
        with open(java_file_path, "w", encoding="utf-8") as f:
            f.write(
                c.format(**{"indent": args.indent, "todo": args.todo})
                .replace("\xa0", " ")
                .replace(" ", " ")
                .replace("<br/>\n", "\n")
            )
